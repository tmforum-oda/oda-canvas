Feature: IGXXX-UCXXX-F003-Upgrade component with additional API
    Background:
        Given Given An example component helm chart 'productcatalog' with '1' API in its 'coreFunction' segment
    ...
    Scenario: Upgrade component with additional API in coreFunction
        Given Given An example component helm chart 'productcatalogadd' with '2' API in its 'coreFunction' segment
