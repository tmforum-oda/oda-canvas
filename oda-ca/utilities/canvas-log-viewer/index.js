...
var myListener = newLineStream( function (message) {
    ...
    if (matches.length == 3) {
        ...
        componentName = labelsList[2]
        namespace = labelsList[1]
        functionName = labelsList[0]
    } else {
...
log.log('canvas', 'oda-controller-ingress-7d4c97c977-p7fw6', 'oda-controller-ingress', logStream, {follow: true, tailLines: 50, pretty: false, timestamps: false})
.catch(err => {
        console.log(err);
        process.exit(1);
})
