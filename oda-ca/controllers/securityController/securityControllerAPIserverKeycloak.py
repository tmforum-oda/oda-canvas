...
def party_role_listener():
    client = ''
    doc = request.json
    logging.debug('security-APIListener received %s', doc)
    party_role = doc['event']['partyRole']
    logging.debug('partyRole = %s', party_role)
        ...
        if client != '':
            if event_type==PARTY_ROLE_CREATION:
                try: # to add the role to the client in Keycloak
                    ...
                    logging.info(format_cloud_event(
                ...
                logging.debug('Update Keycloak for UPDATE')
            else:
                logging.warning(format_cloud_event(
