...
def partyRoleListener():
    ...
    if partyRole['@baseType'] == 'PartyRole':
        ...
        try: # to authenticate and get a token
            ...
            logging.error(formatCloudEvent(str(e), "security-APIListener couldn't determine Keycloak URL"))
            raise

        try: # to get the list of existing clients
            clientList = kc.getClientList(token, kcRealm)
        except requests.HTTPError as e:
        ...
        if client != "":
            if eventType==PARTY_ROLE_CREATION:
                try: # to add the role to the client in Keycloak
                    kc.addRole(partyRole["name"], client, token, kcRealm)
                except requests.HTTPError as e:
                ...
                try: # to add the role to the client in Keycloak
                    kc.delRole(partyRole["name"], client, token, kcRealm)
                except requests.HTTPError as e:
