...
def assert_ids(svc_list, id_list):
    ...
    assert (
        expected_id_list == actual_id_list
...
def test_create_invalid_state(svc_inv, rfmock):
    ...
    try:
        ...
        assert (
            "request.body.state should be equal to one of the allowed values"
            in e.args[0]
        )
...
def test_list_services(svc_inv, rfmock):
    ...
    svcs = svc_inv.list_services(
        component_name="acme-productinventory",
        dependency_name="downstreamproductcatalog",
    )
    ...
    svcs = svc_inv.list_services(
        component_name="bcme-productinventory",
        dependency_name="downstreamproductcatalog",
    )
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            "test_service_inventory_client.py",
            "-s",
            "-W",
