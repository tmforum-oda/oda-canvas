...
def test_find_spec_success(comp_reg, rfmock):
    ...
    rfmock.mock_post(
        "$TOK/realms/odari/protocol/openid-connect/token", f"get_token_success1", 200
    )
    ...
    assert (
        getCharacteristicValue(comps[0]["resourceCharacteristic"], "url")
        == "https://components.ihc-dt.cluster-2.de/r-cat-productcatalogmanagement/tmf-api/productCatalogManagement/v4"
    )
    assert (
        getCharacteristicValue(comps[0]["resourceCharacteristic"], "specification")[0][
            "url"
        ]
        == oas_spec
    )
...
def test_find_spec_no_result(comp_reg, rfmock):
    ...
    rfmock.mock_post(
        "$TOK/realms/odari/protocol/openid-connect/token", f"get_token_success2", 200
    )
...
def test_get_upstream_registries(comp_reg, rfmock):
    ...
    rfmock.mock_post(
        "$TOK/realms/odari/protocol/openid-connect/token", f"get_token_success3", 200
    )
