...
secretsmanagementtype_label = os.getenv('SECRETSMANAGEMENTTYPE_LABEL', "oda.tmforum.org/secretsmanagement")
...
def get_sman_spec(sman_name, sman_namespace):
    ...
    try:
        sman_cr = coa.get_namespaced_custom_object(SMAN_GROUP, SMAN_VERSION, sman_namespace, SMAN_PLURAL, sman_name)
        return sman_cr["spec"]
    except ApiException:
        return None
...
def inject_sidecar(body, patch):
    ...
    if not sman_spec:
        raise kopf.AdmissionError(f"secretsmanagement {sman_cr_name} has no spec.", code=400)
    ...
    if not smanname:
        raise kopf.AdmissionError(f"secretsmanagement {sman_cr_name}: missing name.", code=400)      
    if type != "sideCar":
        raise kopf.AdmissionError(f"secretsmanagement {sman_cr_name}: unsupported type {type}.", code=400)
    if podsel_name and not fnmatch.fnmatch(pod_name, podsel_name):
        raise kopf.AdmissionError(f"secretsmanagement {sman_cr_name}: pod name does not match selector.", code=400)      
    if podsel_namespace and not fnmatch.fnmatch(pod_namespace, podsel_namespace):
        raise kopf.AdmissionError(f"secretsmanagement {sman_cr_name}: pod namespace does not match selector.", code=400)      
    if podsel_serviceaccount and not fnmatch.fnmatch(pod_serviceAccountName, podsel_serviceaccount):
        raise kopf.AdmissionError(f"secretsmanagement {sman_cr_name}: pod serviceAccountName does not match selector.", code=400)      
...
@kopf.on.mutate('pods', labels={"oda.tmforum.org/secretsmanagement": "sidecar"}, operation='CREATE', ignore_failures=True)
...
@kopf.on.mutate('deployments', labels={"oda.tmforum.org/secretsmanagement": "sidecar"}, operation='CREATE', ignore_failures=True)
...
def setupSecretsManagement(sman_namespace:str, sman_name:str, pod_name:str, pod_namespace:str, pod_service_account:str):
    try:
        ...
        if f"{secrets_mount}/" in existing_secret_engines:
            ...
            client.sys.enable_secrets_engine("kv", secrets_mount, options={"version":"2", "description":f"ODA SecretsManagement for {sman_namespace}:{sman_name}"})
...
async def secretsmanagementCreate(meta, spec, status, body, namespace, labels, name, **kwargs):
    ...
    setupSecretsManagement(sman_namespace, sman_name, pod_name, pod_namespace, pod_service_account)
    
    if not implementationReady(body):
        setSecretsManagementReady(sman_namespace, sman_name)
    
    restart_pods_with_missing_sidecar(sman_namespace, pod_name, pod_namespace, pod_service_account)
...
async def secretsmanagementDelete(meta, spec, status, body, namespace, labels, name, **kwargs):
...
def setSecretsManagementReady(namespace, name):
    ...
    try:
        ...
        if e.status == HTTP_NOT_FOUND:
            logger.error(f"setSecretsManagementReady: secretsmanagement {namespace}:{name} not found")
            raise kopf.TemporaryError(f"setSecretsManagementReady: secretsmanagement {namespace}:{name} not found")
        else:
            raise kopf.TemporaryError(f"setSecretsManagementReady: Exception in get_namespaced_custom_object: {e.body}")
    ...
    try:
        ...
        raise kopf.TemporaryError(f"setSecretsManagementReady: Exception in patch_namespaced_custom_object: {e.body}")


@kopf.on.field(SMAN_GROUP, SMAN_VERSION, SMAN_PLURAL, field="status.implementation", retries=5)
        ...
        if status["implementation"]["ready"] == True:
            if "ownerReferences" in meta.keys():
                ...
                if ready != True:  # avoid recursion
                    logger.info(f"patching securitySecretsManagement in component {parent_component_name}")
