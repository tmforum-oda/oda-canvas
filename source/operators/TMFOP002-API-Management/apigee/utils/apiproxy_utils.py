...
def generate_apiproxy_files(
    ...
    if target_url and not target_url.startswith(
        ...
        logger.warning(
            "Target URL '%s' does not start with a valid scheme. Prepending 'http://'.",
            target_url,
        )
    ...
    if template_name:
        if template_name.startswith("http://") or template_name.startswith("https://"):
            ...
            try:
                ...
                remote_policy_file = os.path.join(
                    bundle_path, "apiproxy", "policies", f"{policy_root_name}.xml"
                )
                ...
                logger.info(
                    "Remote policy '%s' saved to %s",
                    policy_root_name,
                    remote_policy_file,
                )
            except Exception as e:
                logger.error(
                    "Failed to download or parse remote template from %s: %s",
                    template_name,
                    e,
                )
        else:
            ...
            if os.path.exists(template_dir):
                logger.info(
                    "Applying additional local template '%s' from %s",
                    template_name,
                    template_dir,
                )
                parts = []
    ...
    for relative_path, template_str in APIPROXY_TEMPLATES.items():
        ...
        if "VerifyAPIKey.Validate.xml" in relative_path and (
            not verify_api_key_step or not location
        ):
            logger.info(
                "Skipping generation of VerifyAPIKey policy file because API key verification is disabled or location is empty."
            )
            continue
        if "SpikeArrest.RateLimit.xml" in relative_path and not spike_arrest_step:
            logger.info(
                "Skipping generation of SpikeArrest policy file because rate limiting is not enabled."
            )
            continue
        if "CORS.xml" in relative_path and not cors_enabled:
            logger.info(
                "Skipping generation of CORS policy file because it is not enabled."
            )
            continue
