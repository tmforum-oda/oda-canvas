...
def apigBind(meta, spec, status, body, namespace, labels, name, **kwargs):
    ...
    apigEndpoint = os.getenv(
        "APIG_ENDPOINT",
        "%s.%s:%d" % (APIG_DEFAULT_INGRESS, namespace, APIG_DEFAULT_PORT),
    )
    ...
    if not (
        status
        and ("apigBind" in status.keys())
        and status["apigBind"]["spec"] == apiSpec
    ):
        ...
        try:
            ...
            apiObj = customObjectsApi.get_namespaced_custom_object(
                GROUP, VERSION, namespace, APIS_PLURAL, meta["name"]
            )

            ingressApi = kubernetes.client.NetworkingV1beta1Api()
            listIngressResp = ingressApi.read_namespaced_ingress(
                apigIngressName, namespace
            )
            ...
            if ingressTarget:
                ...
                apiObj["status"]["apiStatus"]
                apiObj["status"]["apiStatus"]["developerUI"] = (
                    "http://" + ingressTarget + spec["path"] + "/docs/"
                )
                apiObj["status"]["apiStatus"]["ip"] = ingressTarget
                apiObj["status"]["apiStatus"]["name"] = meta["name"]
                apiObj["status"]["apiStatus"]["url"] = (
                    "http://" + ingressTarget + spec["path"]
                )
                apiObj["status"]["implementation"] = {"ready": True}
                patchRslt = customObjectsApi.patch_namespaced_custom_object(
                    GROUP, VERSION, namespace, APIS_PLURAL, meta["name"], apiObj
                )
...
def restCall(host, path, spec):
    ...
    try:
        ...
        if data:
            ...
            logging.info(
                f"Rest api response code: %s, body: %s" % (resp.status, respStr)
            )
            respBody = json.loads(respStr)
        hConn.close()
        if resp.status != 200:
            raise kopf.TemporaryError(
                "Exception when calling rest api, return code: %s\n" % resp.status
            )
        return respBody
