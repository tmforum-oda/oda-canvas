...
def create_or_update_ingress(spec, name, namespace, meta, **kwargs):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"HTTPRoute '{ingress_name}' updated successfully in namespace '{namespace}'.")
            return True
            ...
            if e.status == 404:
                ...
                logger.info(f"HTTPRoute '{ingress_name}' created successfully in namespace '{namespace}'.")
                return True 
            else:
...
def update_httproute_annotations(name, namespace, annotations):
    ...
    try:
        ...
        logger.info(f"HTTPRoute '{ingress_name}' updated with plugins/policies in namespace '{namespace}'.")
        return True
    except ApiException as e:
        logger.error(f"Failed to update plugins/policies for HTTPRoute '{ingress_name}': {e}")
        return False
...
def manage_plugins_from_url(spec, name, namespace, meta):
    ...
    if check_url(template_url):
        ...
        if templates:
            ...
            plugin_names.extend(apply_plugins_from_template(templates, namespace, owner_references))
...
def delete_api_lifecycle(meta, name, namespace, **kwargs):
    ...
    ingress_name = f"kong-api-route-{name}"
    logger.info(f"HTTPRoute '{ingress_name}'  to be automatically deleted as child resource of ExposedAPI '{name}' .")
