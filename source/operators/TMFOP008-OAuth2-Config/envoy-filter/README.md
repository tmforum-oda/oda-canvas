# OAuth2 EnvoyFilter Operator

## Overview

The OAuth2 EnvoyFilter Operator is a component of the ODA Canvas that enforces token-based authentication at the Istio service mesh level. It is built using the Kopf Kubernetes operator framework to manage EnvoyFilter and related Istio resources, ensuring that only properly authorised callers can communicate between ODA Components — without requiring individual components to implement their own OAuth2 authentication logic.

## Key Features

- **Istio Integration**: Dynamically creates EnvoyFilter, ServiceEntry, and DestinationRule resources to inject OAuth2 token verification into the Istio service mesh.
- **Lifecycle Management**: Automates creation, update, and deletion of authentication configuration in response to DependentAPI resource events.
- **Credentials Management**: Works alongside the Credentials Management Operator to obtain and manage OAuth2 client credentials.
...
Run: `kopf run docker/src/oauth2EnvoyfilterOperator.py`
