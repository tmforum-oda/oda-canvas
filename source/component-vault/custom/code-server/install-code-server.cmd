...
cd code-server
helm upgrade --install -n code-server --create-namespace code-server --set existingSecret=code-server-secret ci/helm-chart --set image.repository=mtr.devops.telekom.de/magenta_canvas/public --set image.tag=code-server-with-helm-4.22.0
cd ..
...
kubectl apply -f ../virtualservices/code-server-vs.yaml
