...
def restart_deployment(namespace: str, deployment_name: str):
    ...
    try:
        ...
        print(
            f"Error restarting deployment '{deployment_name}': {str(e)}, continuing..."
        )


def create_or_update_k8s_secret(
    namespace: str, client_id: str, client_secret: str, token_url: str
) -> bool:
    """
    ...
    try:
        ...
        metadata = client.V1ObjectMeta(
            name=secret_name,
            namespace=namespace,
            labels={"app": "componentregistry", "managed-by": "keycloak-setup"},
        )
        ...
        try:
            old_secret = v1.read_namespaced_secret(
