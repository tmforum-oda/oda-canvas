...
if CICD_BUILD_TIME:
    print(f"CICD_BUILD_TIME={CICD_BUILD_TIME}")
if GIT_COMMIT_SHA:
...
def create_clients_users_and_roles(
    ...
    compreg_secret_changed = create_or_update_k8s_secret(
        K8S_NAMESPACE, crmgr_client_id, crmgr_client["secret"], kc.get_token_url()
    )
    ...
    depapiop_secret_changed = create_or_update_k8s_secret(
        K8S_NAMESPACE, depapi_client_id, depapi_client["secret"], kc.get_token_url()
    )
    ...
    if not ui_viewer_role in roles:
        ...
        kc.add_role(
            ui_viewer_role, crmgr_client["id"], description="Role for Compreg UI access"
        )
        roles = kc.get_roles(crmgr_client["id"])
    if not ui_admin_role in roles:
        print(f"Creating {ui_admin_role} role...")
        kc.add_role(
            ui_admin_role, crmgr_client["id"], description="Role for Compreg UI access"
        )
        roles = kc.get_roles(crmgr_client["id"])
    if not sync_role in roles:
        print(f"Creating {sync_role} role...")
        kc.add_role(
            sync_role, crmgr_client["id"], description="Role for Compreg SYNC access"
        )
        roles = kc.get_roles(crmgr_client["id"])
    if not query_role in roles:
        print(f"Creating {query_role} role...")
        kc.add_role(
            query_role, crmgr_client["id"], description="Role for Compreg QUERY access"
        )
        roles = kc.get_roles(crmgr_client["id"])
    ...
    if ui_viewer_user is None:
        ...
        ui_viewer_user = kc.create_user(
            ui_viewer_user_name, ui_viewer_user_init_password
        )
    print("--- USER ----")
    ...
    if not query_role in mapped_depapi_roles:
        print(
            f"Mapping {query_role} role to {depapi_client_id} service account user..."
        )
