        ...
        for char in db_resource.characteristics:
            characteristics.append(
                schemas.Characteristic(
                    name=char.name, value=char.value, valueType=char.valueType
                )
            )
    ...
    if db_resource.related_parties:
        for party in db_resource.related_parties:
            related_parties.append(
                schemas.RelatedPartyRefOrPartyRoleRef(
                    id=party.id,
                    href=party.href,
                    name=party.name,
                    role=party.role,
    ...
    if db_resource.resource_relationships:
        for rel in db_resource.resource_relationships:
            relationships.append(
                schemas.ResourceRelationship(
                    id=rel.id,
                    href=rel.href,
                    relationshipType=rel.relationshipType,
                    resource=schemas.ResourceRefOrValue(
                        id=rel.related_resource_id, href=rel.related_resource_href
                    ),
                )
            )
