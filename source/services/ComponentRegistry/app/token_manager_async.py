...
class TokenManager:
    ...
    def __init__(
        self, token_url, client_id, client_secret, refresh_buffer_seconds=30
    ) -> None:
    ...
    async def _token(self) -> str:
        ...
        try:
            if self._client_id and self._client_secret and self._token_url:
                ...
                async with AsyncOAuth2Client(
                    ...
                    token_data = await oauth_client.fetch_token(
                        self._token_url, grant_type="client_credentials"
                    )
            ...
            raise RuntimeError(f"get_token failed with error message: {e}") from e

    def _is_token_valid(self):
        if not self._access_token or not self._token_expiry:
            return False
        return datetime.now() < (
            self._token_expiry - timedelta(seconds=self._refresh_buffer_seconds)
        )
