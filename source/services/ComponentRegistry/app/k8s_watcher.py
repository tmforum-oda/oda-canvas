...
class WatchResourceChangesThread(Thread):
    ...
    def __init__(
        ...
        kind,
        group=None,
        namespaces: Optional[List[str]] = DEFAULT_NAMESPACES,
        *args,
        **kwargs,
    ):
    ...
    def run(self):
        while True:
            try:
                ...
                print(
                    f"Exception in WatchResourceChangesThread for {self._kind}: {e}. Restarting watch..."
                )
                time.sleep(5)

    def run_inner(self):
        api = self._dyn_client.resources.get(
            api_version=self._api_version, group=self._group, kind=self._kind
        )
        ...
        while True:
            print(
                f'Start watching {self._kind} with resource_version "{resource_version}"'
            )
            try:
                for event in api.watch(
                    ...
                    if ev_type != "BOOKMARK":
                        if self.check_namespace_match(namespace):
                            print(
                                f"[WATCHER] {ev_type} {kind} {namespace}:{name}: {resource_version}"
                            )
                            self._queue.put(
                                {
                                    "type": ev_type,
                ...
                if err.status == 410:
                    print(
                        f"ERROR: The requested resource version {resource_version} is no longer available."
                    )
                    result = api.get(namespace=self._search_namespace)
                    resource_version = (
                        result.metadata.resourceVersion
                    )  # resource version of the list query, not the items
...
class K8SWatcher:
    ...
    def update(self, event):
        ...
        self._updated_versions[(kind, namespace, name)] = {
            "kind": kind,
            "namespace": namespace,
            "name": name,
            "rv": rv,
    ...
    def add_watch(self, api_version, kind, group=None, namespaces=DEFAULT_NAMESPACES):
        watch_thread = WatchResourceChangesThread(
            self._dyn_client,
            self._queue,
    ...
    async def send_callback(self, info):
        ...
        old_rv = (
            None
            if (kind, namespace, name) not in self._sent_versions
