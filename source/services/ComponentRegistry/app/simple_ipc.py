...
class SimpleIPC:
    ...
    def init_shm(cls):
        ...
        data = (
            ...
            + [0] * MAX_PROCESSES
            + [0] * MAX_PROCESSES
            + ["a" * MAX_DATA_SIZE] * MAX_PROCESSES
        )
        myshl_init = shared_memory.ShareableList(data)
        try:
            myshm = shared_memory.SharedMemory(
                name=cls._shm_name, create=True, size=myshl_init.shm.size
            )
    ...
    def init(self):
        ...
        self.register_own_process()
        print(
            f"Process {self._process_id} registered as process number {self._process_num} in {self._shm_name}"
        )
    ...
    def shutdown(self):
        if self._process_num is not None:
            ...
            print(
                f"Process {self._process_id} unregistered from process number {self._process_num} in {self._shm_name}"
            )
