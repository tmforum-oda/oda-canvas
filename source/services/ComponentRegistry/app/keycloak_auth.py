...
KEYCLOAK_URL = os.getenv(
    "KEYCLOAK_URL", "https://canvas-keycloak.ihc-dt.cluster-2.de/auth/realms/odari"
)
...
KEYCLOAK_VALID_ISSUERS_str = os.getenv(
    "KEYCLOAK_VALID_ISSUERS",
    "http://canvas-keycloak.canvas.svc.cluster.local:8083/auth/realms/odari",
)
KEYCLOAK_VALID_ISSUERS_array = set(
    [iss.strip() for iss in KEYCLOAK_VALID_ISSUERS_str.split(",")]
)
...
async def get_oidc_config(issuer: str) -> Dict[str, Any]:
    ...
    if (
        ...
        and issuer in _cache_expiry
        and datetime.utcnow() < _cache_expiry[issuer]
    ):
