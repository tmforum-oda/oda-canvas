...
async def get_components_by_oas_specification(
    ...
    """
    Get all Components, but only include ExposedAPIs that match the given oas_specification.
    """
    all_components = crud.ComponentCRUD.get_all(db)
    filtered_components = []
    for component in all_components:
        matching_apis = [api for api in getattr(component, "exposed_apis", []) if getattr(api, "oas_specification", None) == oas_specification]
        if matching_apis:
            # Create a copy of the component with only the matching APIs
...
def resource_to_component_registry_create(resource: Resource_Create) -> models.ComponentRegistryCreate:
    ...
    return models.ComponentRegistryCreate(
        name=resource.name,
        url=url or "http://localhost:8082",  # Default URL if none provided
        type=registry_type,
