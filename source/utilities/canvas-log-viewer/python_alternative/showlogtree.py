...
LINE_RX = re.compile(f"{TIME_RX}{sp}{LOGGER_RX}{sp}{LEVEL_RX}{sp}({COMPONENT_RX}{RESOURCE_RX}{HANDLER_RX}{FUNCTION_RX}{sp}{SUBJECT_RX}{sp}|{RESOURCE_ONLY_RX}{sp}|){MESSAGE_RX}")
LINE_RO_RX = re.compile(f"{TIME_RX}{sp}{LOGGER_RX}{sp}{LEVEL_RX}{sp}{RESOURCE_ONLY_RX}{sp}{MESSAGE_RX}")
...
def create_log_tree(filename):
    ...
    for entry in entries:
        ...
        message = f'{entry["subject"]}: {entry["message"]}' if entry["subject"] else entry["message"]
...
def show_log_tree(filename):
    logTree = create_log_tree(filename)
