...
function generateLink(context,query,skip,limit,type) {
  ...
  // normalize basePath: remove trailing slashes except when path is root '/'
  let normalizedBasePath = basePath.replace(/\/+$/, '')
  // fallback: if normalization produced empty string, keep single  '/'
  if (normalizedBasePath === '') normalizedBasePath = '/'

  const hostPath = swaggerUtils.getHostPath(context)
  const url = new URL(hostPath)
  
  logger.debug("generateLink: basePath=" + basePath + " normalizedBasePath=" + normalizedBasePath + " hostPath=" + hostPath + " url=" + url)
  ...
  if(!normalizedBasePath.startsWith('/')) linkPath = linkPath + '/'
  linkPath = linkPath + normalizedBasePath
