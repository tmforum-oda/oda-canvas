...
class Service {
  ...
  static async index(args, context) {
    ...
    try {
      let resp = await db.findMany(resourceType, query)

      let doc = resp[0]
      let totalSize = resp[1]

      if (totalSize === 0) {
        logger.debug("index:: resourceType=" + resourceType + " no items found")
        args = this.generateCreatePayload(args,resourceType)
        await this.create(args, context)
        resp = await db.findMany(resourceType, query)
        doc = resp[0]
        totalSize = resp[1]
      }
  ...
  static async create(args, context) {
    ...
    try {
      ...
      if (isAsyncBehaviour(resourceType,context)) {
        const {header, body} = await this.generateAsyncResponse(context, payload)
        headerParams = [...headerParams, ...header]
        notify.publish(context, payload)
        code = 202
        await db.create(config.MONITOR, body)
        //if(doc?.href) headerParams.push({Location: doc.href})
        //return Service.successResponse(responsePayload, 202, headerParams)
      }
      ...
      logger.info("create:: resourceType=" + resourceType + ": error=" + error)
      throw Service.rejectResponse(error)
    }
  ...
  static generateCreatePayload(args, resourceType) {
    switch(resourceType) {
      case "IntentReport":
        return {
            "name": "MyIntent Report",
            "description": "Auto-generated Intent Report",
            "expression": {
              "@type": "JsonLdExpression",
              "expressionValue": {
                "@context": {
                   "voc": "https://tmforum.org/vocab/intent/v1",
  ...
  static async generateAsyncResponse(context, payload) {
    ...
    try {
      body = await createMonitor(context, payload, this.create)

      const link = { Link: '<' + body.href + '>; rel=related; title=monitor' }
      header.push(link)
