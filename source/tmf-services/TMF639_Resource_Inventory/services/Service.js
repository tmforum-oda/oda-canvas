...
class Service {
   ...
   */  static applyQuery(resources, args, context) {
    ...
    // If args.limit is not available, try to get from request context
    if (isNaN(limit) && context.request?.query?.limit) {
      limit = parseInt(context.request.query.limit);
    }
    
    // Try queryParams as backup
    if (isNaN(limit) && context.request?.queryParams?.limit) {
      limit = parseInt(context.request.queryParams.limit);
    }
    
    if (isNaN(limit)) {
      limit = result.length; // default to all results
    }
  ...
  static applyFields(resource, fields) {
    ...
    // Always include @type and id if not explicitly requested
