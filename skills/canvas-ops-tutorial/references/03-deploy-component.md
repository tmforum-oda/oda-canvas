...
## Step 5: Explain what is happening

Whilst the component is deploying, this is a good time to explain what is happening. Use the '01-about-oda-canvas.md' reference to explain to the user what the Canvas Operators do when you deploy a component.

## Step 6: Query Deployment

Query the component status: The component-name is made up of <release-name>-<component-name>. For example `pc1-productcatalogmanagement`.

```bash
kubectl get component <component-name> -n components
```

Don't offer to monitor the component status until it reaches `Complete`: Instead offer to replay the query above.

If status does not reach `Complete` within 5 iterations, suggest:
...
If the component status is still not `Complete` then start troubleshooting.

## Step 7: Explore the Decomposed Resources
...
kubectl get identityconfigs -n components -l oda.tmforum.org/componentName=<component-name>
kubectl get secretsmanagements -n components -l oda.tmforum.org/componentName=<component-name>
```
